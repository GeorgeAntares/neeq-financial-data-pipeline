...
from cashflow_features import FEATURE_COLS_MODEL, labeled_cashflow_frame, split_and_scale
...
def main():
    ...
    y = df['healthy'].values
    X_tr, X_val, X_test, y_tr, y_val, y_test, scaler = split_and_scale(X, y)

    X_train_tensor = torch.FloatTensor(X_tr).to(device)
    y_train_tensor = torch.FloatTensor(y_tr).to(device)
    ...
    print(f"  训练集 / Train: {len(X_tr)} samples")
    print(f"  验证集 / Val: {len(X_val)} samples (early stopping)")
    ...
    with torch.no_grad():
        all_prob = model(torch.FloatTensor(scaler.transform(X)).to(device)).cpu().numpy()
    ...
    print("=" * 60)
    print("Hold-out test metrics above; scaler fitted on train only, early stopping on val.")
